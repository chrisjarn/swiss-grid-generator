...
test("typography layout plan reflows across the full stacked row height before advancing to the next column", () => {
  ...
  assert.match(source, /const\s+getReflowLineSlot\s*=\s*\([\s\S]*?rowLayouts:\s*readonly\s+ReflowRowLayout\[\],[\s\S]*?lineIndexWithinColumn:\s*number,[\s\S]*?lineStep:\s*number,/)
  ...
  assert.match(source, /const\s+lineSlot\s*=\s*getReflowLineSlot\(reflowRowLayouts,\s*lineIndexWithinColumn,\s*lineStep\)/)
  assert.match(source, /const\s+lineTopY\s*=\s*origin\.y\s*\+\s*baselineStep\s*\+\s*verticalStartOffset\s*\+\s*lineSlot\.yOffset/)
