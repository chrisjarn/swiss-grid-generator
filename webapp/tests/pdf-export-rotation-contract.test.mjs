...
test("typography layout plan reflows across the full stacked row height before advancing to the next column", () => {
  ...
  assert.match(source, /const\s+reflowCapacityHeight\s*=\s*moduleHeightForBlock\s*\+\s*\(columnReflow\s*&&\s*rowSpan\s*>\s*0\s*\?\s*gutterY\s*:\s*0\)/)
  assert.match(source, /const\s+maxLinesPerColumn\s*=\s*Math\.max\(1,\s*columnReflow[\s\S]*?\?\s*getReflowLineCapacityForHeight\(reflowCapacityHeight,\s*lineStep\)[\s\S]*?:\s*getLineCapacityForHeight\(moduleHeightForBlock,\s*lineStep,\s*firstLineHeight\)\)/)
  ...
  assert.match(source, /const\s+lineSlotBottomY\s*=\s*lineTopY\s*\+\s*lineStep/)
