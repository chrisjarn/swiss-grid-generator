...
test("newspaper reflow uses interior row gutters as continuous column space", () => {
  const lines = ["L1", "L2", "L3", "L4", "L5"].map((text, index) => ({
    text,
    sourceStart: index * 2,
    sourceEnd: index * 2 + 2,
  ...
  const { plans } = buildTypographyLayoutPlan({
    ...
    isTextReflowEnabled: () => true,
    isSyllableDivisionEnabled: () => false,
    getOriginForBlock: (_key, fallbackX, _fallbackY) => ({ x: fallbackX, y: 0 }),
    createTextContext: () => ({}),
    wrapText: () => lines,
    textAscent: () => 8,
    textDescent: () => 2,
  ...
  assert.deepEqual(
    plan.commands.map((command) => command.y),
    [18, 28, 38, 48, 58],
  )
  assert.deepEqual(plan.guideRects, [
    { x: 0, y: 10, width: 50, height: 50 },
    { x: 60, y: 10, width: 50, height: 50 },
  ])
})
